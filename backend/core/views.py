...
class RecicleMaterialsStatisticsViewByState(APIView):
    ...
    def get(self, request, state: str or None):
        ...
        for city in recicle_atlas.by_city.keys():
            ...
            try:
                occurrence_by_state[
                    recicle_atlas.by_city[city].get("state", str())
                ].append(recicle_atlas.by_city[city])
            except KeyError:
                occurrence_by_state[recicle_atlas.by_city[city].get("state", str())] = [
                    recicle_atlas.by_city[city]
                ]
        ...
        for state in df["state"]:
            if state == "RJ":
                ...
                graph = px.histogram(
                    state_data,
                    y="state",
                    x=["paper", "plastic", "glass", "steel", "cooperatives"],
...
class RegisterUser(APIView):
    ...
    def post(self, request, format=None):
        ...
        if user_form.is_valid():
            ...
            return Response(
                [{"errors": user_form.errors}], status=status.HTTP_400_BAD_REQUEST
            )
...
class AuthLoginView(APIView):
    ...
    def post(self, request, format=None):
        ...
        if user is not None:
            ...
            return Response(
                {"errors": "Senha ou usuários inválido"},
...
class RecycleBalanceView(APIView):
    ...
    def post(self, request):
        ...
        recicle_balance_occurrence = RecycleBalance(
            user_id_occurrence=user_id, material_id=material_id, mesure=mesure
        ).save()
    ...
    def put(self, request):
        ...
        if not id_:
            errors["errors"].append(
                "Necessário adicionar o id da ocorrência da balança"
            )
            return Response([errors])
        object_ = RecycleBalanceStorage()
        ...
        if not recicle_balance_occurrence:
            return Response(
                [{"errors": "Não foi possível encontrar a ocorrência da balança"}]
            )
        if len(errors["errors"]) > 0:
            return Response([errors])
        has_changes = bool()
...
class OperationsBalanceView(APIView):
    ...
    def post(self, request):
        ...
        for recycle_balance in user_open_list["recicle_balance_array"]:
            ...
            control = object_.deactivate_recycle_balance_occurrence(
                recycle_balance["id"]
            )
        ...
        if len(recyle_balance_occurrences) > 0:
            _ = OperationsBalance(
                ...
                operation_type=1,
                balance_ids=recyle_balance_occurrences,
            ).save()
            _ = object_user.alter_user_value(user_id, user_open_list["final_value"], 1)
            return Response(
                [{"errors": "Operação Finalizada"}], status=status.HTTP_204_NO_CONTENT
            )
        else:
...
class UserControlView(APIView):
    ...
    def get(self, request):
        ...
        if possible_user:
            ...
            return Response(
                [{"errors": "Usuário não encontrado ou não possui nenhuma operação"}]
            )
...
class OperationsBalanceByUserView(ListAPIView):
    serializer_class = UserOperationSerializer

    def get_queryset(self, *args, **kwargs):
        # TODO change user filter to request.user.id
        user_id = self.kwargs["user_id"]
        balances = RecycleBalance.objects.filter(user_id_occurrence=user_id).values()
        dict_balances = {balance["id"]: balance for balance in balances}
        operations = list(OperationsBalance.objects.filter(user_id=user_id).values())
        materials = MaterialsToRecycle.objects.values()
        materials_dict = {material["id"]: material for material in materials}
        for operation in operations:
            balances = []
            for balance in operation["balance_ids"]:
                balance = dict_balances[balance["recycle_balance_id"]]
                balance["material"] = materials_dict[balance.get("material_id")]
